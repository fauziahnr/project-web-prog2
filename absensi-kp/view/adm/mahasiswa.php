<h3 class='page-header'>Daftar Mahasiswa KP</h3>
<?php 
if (isset($_GET['st'])) {
        if ($_GET['st']==1) {
    ...
    <?php 
        if (isset($_GET['id_siswa'])) {
            if ($_GET['id_siswa']!=="") {
                ...
                include './view/adm/edit_mahasiswa.php';
            } else {
...
$q = "SELECT*FROM detail_user ORDER BY  id_user, sklh_user ASC LIMIT $limitvalue, $limit";
//jika user nakal paging lebih dari data yg dimiliki
$cek_page = $conn->query($q);
if ($cek_page->rowCount() != 0) {
    if ($r = $conn->query($q)) {
    ...
    if ($r->rowCount()!==0) {
            echo "<table class='table table-striped' style='width:70%'>
                    ...
                    $no=0;
                    while ($get_siswa = $query_siswa->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<tr>
                                ...
                                <td><a href='mahasiswa&id_siswa=$id_siswa' title='Edit $name'>Edit info</a> &bullet; <a style='cursor:pointer' onclick='hapusSiswa($id_siswa)' >Hapus</a></td>
                            </tr>";
                    }
                   // $conn->close();
                    echo "</tbody></table>";
        ...
        function hapusSiswa(id_siswa) {
            ...
            swal({
                ...
                closeOnConfirm: true
                    ...
                    window.location.href="./model/proses.php?del_mahasiswa="+id;
                });
        }
    </script>