<?php
	if (isset($_SESSION['sw'])) {
		...
		echo "<h1 class='page-header'>Welcome, $name</h1>";
			...
			if ($conn->query("SELECT*FROM data_absen WHERE id_user='$id_user'")->rowCount()!==0) {
				$no=0;
			 	$query_month=$conn->query("SELECT*FROM bulan ORDER BY id_bln ASC");
			 	while ($get_month=$query_month->fetch(PDO::FETCH_ASSOC)) {
			      ...
			      if ($cek!==0) {
			        ...
			        echo "<div class='table-responsive'>
			           <table class='table table-striped'>
			            ...
			            <tbody>";
			          $no=0;
			          while ($get_absen=$query_absen->fetch(PDO::FETCH_ASSOC)) {
			            $no++;
		...
		$sql = "SELECT * FROM detail_user WHERE id_user='$id_siswa'";
		$query = $conn->query($sql);
		$get_user = $query->fetch(PDO::FETCH_ASSOC);
		$name = $get_user['name_user'];
		$id_user = $get_user['id_user'];
